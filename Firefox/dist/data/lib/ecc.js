...
var ecc = (function (window, undefined) {
  ...
  */
  ...
  ecc.generate = function (type, curve) {
    ...
    } else throw "eccjs: generate: Unknown type";
  ...
  ecc.decrypt = function (deckey, ciphertext) {
    ...
    if (!key) key = kem.$keys[obj.tag] = kem.unkem(sjcl.codec.hex.toBits(obj.tag));
  ...
  ecc.sign = function (sigkey, text, hash) {
    ...
    if (!key) key = cache.sig[sigkey] = dsa.importSecret(sigkey);

    //hash first
    if (hash !== false) text = sha256.hash(text);
  ...
  ecc.verify = function (verkey, signature, text, hash) {
    ...
    if (!key) key = cache.ver[verkey] = dsa.importPublic(verkey);
  ...
  function eccAPI(algoName) {
    ...
    if (!algo) throw new Error("Missing ECC algorithm: " + algoName);
    return {
      generate: function generate(curve) {
        var keys = algo.generateKeys(curve, 1);
        keys.pub.$curve = curve;
        keys.sec.$curve = curve;
        return keys;
      },
      importPublic: function importPublic(keyStr) {
        var key = extract(keyStr);
        return new algo.publicKey(key.curve, sjcl.codec.hex.toBits(key.hex));
      },
      importSecret: function importSecret(keyStr) {
        var key = extract(keyStr);
  ...
  function exportPublic(keyObj) {
    var obj = keyObj.get();
    return keyObj.$curve + sjcl.codec.hex.fromBits(obj.x) + sjcl.codec.hex.fromBits(obj.y);
  }
  ...
  function hashAPI(algoName) {
    ...
    if (!algo) throw new Error("Missing hash algorithm: " + algoName);
    return {
  ...
  //publicise
  if (typeof module !== 'undefined' && module.exports) module.exports = ecc;else window.ecc = ecc;
